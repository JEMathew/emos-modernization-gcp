...
import {
  ...
  LockKeyhole, Printer, Scale, ShieldCheck, Sparkles,
} from 'lucide-react';
...
import { buildSampleDecisionBrief, downloadSampleDecisionBrief } from '../lib/sampleDecisionBrief';
...
export const PublicSandboxPage: React.FC<PublicSandboxPageProps> = ({ onNavigate }) => {
  ...
  const openPrintableBrief = () => {
    const url = URL.createObjectURL(new Blob([buildSampleDecisionBrief(workload)], { type: 'text/html;charset=utf-8' }));
    ...
    <div className="emos-landing min-h-screen bg-[var(--emos-bg)] text-[var(--emos-text-primary)]">
      <header className="sticky top-0 z-40 border-b border-[var(--emos-border-subtle)] bg-[var(--emos-bg-secondary)]/90 backdrop-blur-xl">
        <div className="mx-auto flex h-16 max-w-[88rem] items-center justify-between gap-3 px-4 sm:px-6 lg:px-8">
          ...
          <div className="flex items-center gap-2"><ThemeSelector /><button onClick={() => signInWithGoogle()} className="rounded-xl bg-[#A88554] px-4 py-2 text-sm font-semibold text-black">Continue With Google</button></div>
        </div>
      </header>
          ...
          <div className="grid gap-7 lg:grid-cols-[0.78fr_1.22fr] lg:items-start">
            <div>
              ...
              <div className="mt-5 flex flex-wrap gap-2">
                <button onClick={() => downloadSampleDecisionBrief(workload)} className="inline-flex min-h-11 items-center gap-2 rounded-xl bg-[#A88554] px-4 text-sm font-semibold text-black"><Download className="h-4 w-4" />Download Brief</button>
            ...
            <div className="overflow-hidden rounded-[1.75rem] border border-[var(--emos-border-strong)] bg-[var(--emos-surface)] shadow-2xl">
              ...
              <div className="min-h-[33rem] p-5 sm:p-7">
                <AnimatePresence initial={false}>
                  <motion.div key={step} initial={reduceMotion ? false : { opacity: 0, x: 18 }} animate={reduceMotion ? undefined : { opacity: 1, x: 0 }} exit={reduceMotion ? undefined : { opacity: 0, x: -12 }} transition={{ duration: 0.22 }}>
                    ...
                    {step === 1 && <section><p className="text-xs font-semibold text-[var(--emos-accent-text)]">Start With What You Have</p><h2 className="mt-2 font-serif text-3xl font-semibold">Evidence Is Partial—and Still Useful</h2><div className="mt-5 grid gap-3 sm:grid-cols-2">{dimensions.map(([label, fields]) => { const known = fields.filter((field) => field.status === 'known').length; return <div key={label} className="rounded-xl border border-[var(--emos-border-subtle)] bg-[var(--emos-bg-secondary)] p-4"><div className="flex items-center justify-between"><strong className="text-sm">{label}</strong><span className="font-mono text-xs text-[var(--emos-accent-text)]">{known}/{fields.length}</span></div><div className="mt-3 h-1.5 rounded-full bg-[var(--emos-border-subtle)]"><motion.div className="h-full rounded-full bg-[#A88554]" initial={reduceMotion ? false : { width: 0 }} animate={{ width: `${(known / fields.length) * 100}%` }} transition={{ duration: 0.55 }} /></div></div>; })}</div><div className="mt-5 rounded-xl border border-amber-500/25 bg-amber-500/10 p-4 text-sm text-amber-800 dark:text-amber-200"><strong>Critical gaps:</strong> {readiness.criticalGaps.join(' · ')}</div></section>}
                    ...
                    {step === 3 && <section><p className="text-xs font-semibold text-[var(--emos-accent-text)]">Deterministic Readiness Result</p><h2 className="mt-2 font-serif text-3xl font-semibold">Evidence Before Approval</h2><div className="mt-6 rounded-2xl border border-amber-500/30 bg-amber-500/10 p-5"><div className="flex flex-wrap items-end justify-between gap-3"><div><p className="font-mono text-4xl font-semibold text-amber-700 dark:text-amber-300">{readiness.completeness}%</p><p className="mt-1 text-xs text-[var(--emos-text-muted)]">Current Evidence Completeness</p></div><div className="rounded-full border border-amber-500/30 bg-[var(--emos-surface)] px-3 py-1.5 font-mono text-xs font-bold text-amber-700 dark:text-amber-300">{readiness.decisionReadiness}</div></div><div className="mt-5 h-2 rounded-full bg-[var(--emos-border-subtle)]"><motion.div className="h-full rounded-full bg-amber-500" initial={reduceMotion ? false : { width: 0 }} animate={{ width: `${readiness.completeness}%` }} transition={{ duration: 0.7 }} /></div><div className="mt-2 flex justify-between text-[11px] text-[var(--emos-text-muted)]"><span>0%</span><span>{EMOS_FACTS.readinessThreshold}% Minimum</span><span>100%</span></div></div><div className="mt-5 flex gap-3 rounded-xl border border-rose-500/25 bg-rose-500/10 p-4"><AlertCircle className="mt-0.5 h-5 w-5 shrink-0 text-rose-500" /><div><strong className="text-sm">No 6R Disposition Should Be Approved Yet</strong><p className="mt-1 text-xs leading-5 text-[var(--emos-text-secondary)]">Reaching {EMOS_FACTS.readinessThreshold}% is necessary, but unresolved dependency, TCO or target-state gaps can still block readiness. Validate the named gaps, rerun the gate and request human review.</p></div></div><div className="mt-5 flex items-center gap-2 text-xs text-[var(--emos-text-muted)]"><ShieldCheck className="h-4 w-4 text-emerald-500" />Model confidence cannot override this calculated gate.</div></section>}
                  </motion.div>
                </AnimatePresence>
              </div>
              ...
              <div className="flex items-center justify-between border-t border-[var(--emos-border-subtle)] bg-[var(--emos-bg-secondary)] p-4">
                ...
                {step < STEPS.length - 1 ? <button onClick={() => setStep((current) => Math.min(STEPS.length - 1, current + 1))} className="inline-flex min-h-10 items-center gap-2 rounded-xl bg-[#A88554] px-4 text-sm font-semibold text-black">Next <ArrowRight className="h-4 w-4" /></button> : <button onClick={() => signInWithGoogle()} className="inline-flex min-h-10 items-center gap-2 rounded-xl bg-[#A88554] px-4 text-sm font-semibold text-black">Use Your Portfolio <FileCheck2 className="h-4 w-4" /></button>}
              </div>
            </div>
          </div>
        </section>
