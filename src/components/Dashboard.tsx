...
export const Dashboard: React.FC<DashboardProps> = ({ user }) => {
  ...
  const handleSaveNew = async ({
    ...
    try {
      // 1. Generate Gemini AI assessment & metadata in parallel
      ...
      const attributes = chatRes.attributes || extractAssessmentAttributes(chatRes.response);
      ...
      const newInteraction: Interaction = {
        ...
        evidenceCompleteness: attributes.evidenceCompleteness ?? matchedWorkload?.evidenceCompleteness,
  ...
  const handleSendFollowUp = async (interactionId: string, message: string) => {
    ...
    try {
      ...
      const updatedAttrs = response.attributes || extractAssessmentAttributes(response.response);
  ...
  return (
    <div className="min-h-screen bg-[#0A0A0A] text-[#D4D4D4] flex flex-col font-sans selection:bg-[#A88554] selection:text-black">
      <Navbar
        user={user}
      ...
      <div className="flex-1 flex flex-col lg:flex-row overflow-hidden max-w-7xl w-full mx-auto border-x border-[#1E1E1E]">
        <HistorySidebar
