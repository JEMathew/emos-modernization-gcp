        ...
        {activeSection === 'home' && <section className="landing-scene landing-hero relative" aria-labelledby="landing-title">
          ...
          <div className="relative mx-auto grid max-w-[88rem] gap-10 px-4 py-12 sm:px-6 lg:grid-cols-[1.1fr_0.9fr] lg:items-center lg:gap-12 lg:px-8 lg:py-16">
            <Reveal>
              ...
              <div className="mt-9 flex flex-col gap-3 sm:flex-row sm:flex-wrap sm:items-center">
                <a href="/sandbox" onClick={(event) => navigate(event, '/sandbox')} className="group inline-flex min-h-[50px] items-center justify-center gap-3 rounded-xl bg-gradient-to-r from-[#A88554] to-[#E5C492] px-6 text-sm font-semibold text-black shadow-lg transition-all hover:-translate-y-0.5 sm:text-base"><Sparkles className="h-5 w-5" />Try the Public Sandbox<ArrowRight className="h-4 w-4 transition-transform group-hover:translate-x-1" /></a>
                <button id="primary-google-signin-btn" aria-describedby="portfolio-signin-privacy-note" onClick={handleSignIn} disabled={isSigningIn} className="group flex min-h-[50px] items-center justify-center gap-3 rounded-xl border border-[var(--emos-border-strong)] bg-[var(--emos-surface)] px-5 text-sm font-semibold transition-all hover:-translate-y-0.5 disabled:opacity-75"><GoogleIcon />{isSigningIn ? 'Authenticating…' : 'Sign In to Use Your Portfolio'}</button>
                <button id="learn-more-walkthrough-btn" onClick={onOpenWalkthrough} className="inline-flex min-h-[44px] items-center justify-center gap-2 rounded-xl px-3 text-sm font-semibold text-[var(--emos-text-secondary)] transition-colors hover:bg-[var(--emos-surface-hover)] hover:text-[var(--emos-text-primary)] hover:underline hover:underline-offset-4"><CirclePlay className="h-4 w-4 text-[var(--emos-accent)]" />Watch Product Tour</button>
              </div>
              <p id="portfolio-signin-privacy-note" className="mt-3 flex max-w-2xl items-start gap-2 text-xs leading-5 text-[var(--emos-text-muted)]"><Lock className="mt-0.5 h-3.5 w-3.5 shrink-0 text-[var(--emos-accent)]" /><span>Google Sign-In keeps saved assessments in your account-isolated EMOS workspace. <a href="/privacy" onClick={(event) => navigate(event, '/privacy')} className="font-semibold text-[var(--emos-accent-text)] underline underline-offset-4">Review data handling</a>.</span></p>
          ...
          <div className="relative mx-auto max-w-[88rem] px-4 pb-12 sm:px-6 lg:px-8 lg:pb-16" aria-label="EMOS capability horizon">
            ...
            <div className="grid gap-3 lg:grid-cols-3">
              {CAPABILITY_HORIZON.map(({ label, status, stages }) => <div key={`hero-${label}`} className={['rounded-2xl border p-4 backdrop-blur', PHASE_CARD_STYLES[status]].join(' ')}><div className="flex items-center justify-between gap-3"><p className="font-serif text-base font-semibold">{label}</p><span className={['rounded-full border px-2 py-1 font-mono text-[9px] font-bold tracking-wider', STATUS_STYLES[status]].join(' ')}>{status}</span></div><p className="mt-3 text-xs leading-5 text-[var(--emos-text-secondary)]">{stages.join(' · ')}</p></div>)}
            </div>
