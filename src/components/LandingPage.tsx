      ...
      <main id="top" className="relative overflow-hidden">
        ...
        {activeSection === 'founder' && <section id="founder" className="landing-scene landing-founder bg-[var(--emos-bg-secondary)]">
          <div className="mx-auto grid max-w-[88rem] gap-8 px-4 py-14 sm:px-6 sm:py-16 lg:grid-cols-[0.72fr_1.28fr] lg:items-center lg:px-8">
            <Reveal><div className="rounded-3xl border border-[var(--emos-border-strong)] bg-[var(--emos-surface)] p-6 shadow-xl"><div className="flex items-center justify-between"><div className="flex h-14 w-14 items-center justify-center rounded-2xl bg-gradient-to-tr from-[#876637] to-[#E5C492] font-serif text-2xl font-semibold text-black">JM</div><a href="https://www.linkedin.com/in/jincenmathew/" target="_blank" rel="noreferrer" aria-label="Connect With Jincen E Mathew on LinkedIn" title="Connect With Jincen E Mathew on LinkedIn" className="flex h-11 w-11 items-center justify-center rounded-xl border border-[#0A66C2]/30 bg-[#0A66C2]/10 text-[#0A66C2] transition-all hover:-translate-y-0.5 hover:bg-[#0A66C2] hover:text-white focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-[#0A66C2]"><Linkedin className="h-5 w-5" aria-hidden="true" /></a></div><p className="mt-5 text-xs font-semibold uppercase tracking-[0.16em] text-[var(--emos-accent-text)]">Product Builder</p><h2 className="mt-2 font-serif text-3xl font-semibold">Jincen E Mathew</h2><p className="mt-1 text-sm leading-6 text-[var(--emos-text-muted)]">Product Builder · Bengaluru · 15+ years taking enterprise products from opportunity to scale</p><dl className="mt-5 grid gap-4 border-t border-[var(--emos-border-subtle)] pt-5 sm:grid-cols-2 lg:grid-cols-1">{[
              ['End-to-end ownership', 'Business value & opportunity validation → Design → Build → Production-grade release → Launch & positioning → Scale'],
              ['Oracle · 5 years', 'Led GTM for Oracle Engineered Systems and the Oracle technology stack'],
              ['Tally · 3+ years', 'Built and launched products across new markets and countries'],
              ['Boeing · 6 years', 'Owned a 0→1 modernization-governance product end to end, from business case to scale, across an estate of 5,000+ applications and data platforms. Led modern data platforms end to end: a cloud data warehouse on GCP and a data lakehouse on AWS.'],
              ['Autodesk · 1+ year', 'Leading data products supporting product-led growth and GTM initiatives'],
              ['Community', 'Established the first product management communities of practice at Tally and Boeing India'],
            ...
            <Reveal delay={0.1}><div><h2 className="font-serif text-4xl font-semibold leading-tight sm:text-5xl">Why EMOS exists</h2><div className="mt-5 space-y-4 text-base leading-8 text-[var(--emos-text-secondary)]"><p>At Boeing, the hard part of modernization wasn&apos;t executing a 6R disposition once it was chosen. It was establishing whether the evidence justified committing millions of dollars and years to changing a system.</p><p className="font-serif text-3xl font-semibold leading-tight text-[var(--emos-text-primary)]">EMOS grew from that problem.</p></div><div className="mt-8 overflow-hidden rounded-3xl border border-[var(--emos-border-strong)] bg-[var(--emos-surface)] shadow-xl"><video className="aspect-video w-full bg-[var(--emos-bg-tertiary)]" controls preload="none" poster="/assets/emos/emos-why-exists-poster.webp" aria-label="Why EMOS exists narrated video"><source src="/assets/emos/emos-why-exists.webm" type="video/webm" /><source src="/assets/emos/emos-why-exists.mp4" type="video/mp4" /><track kind="captions" src="/assets/emos/emos-why-captions.en.vtt" srcLang="en" label="English" default />Your browser does not support the video tag.</video></div><div className="mt-6 space-y-4 text-base leading-8 text-[var(--emos-text-secondary)]"><p>EMOS helps enterprises assess modernization options against evidence, make governed decisions and carry those decisions into execution.</p><p>Hyperscaler tools are built to move workloads onto their own clouds. EMOS is vendor-agnostic: the evidence decides the destination, including whether to move at all.</p><p className="text-sm leading-7">Design partners work directly with me to shape the operating model and set the roadmap.</p></div><a href="https://mail.google.com/mail/?view=cm&fs=1&to=jeasom@gmail.com&su=EMOS%20Design%20Partner%20Interest&body=Hi%20Jincen%2C%0A%0AI%27m%20interested%20in%20becoming%20an%20EMOS%20design%20partner." target="_blank" rel="noreferrer" aria-label="Become an EMOS design partner by email" className="group mt-7 inline-flex min-h-[50px] items-center justify-center gap-2 rounded-xl bg-gradient-to-r from-[#A88554] to-[#E5C492] px-6 text-sm font-semibold text-black shadow-lg transition-all hover:-translate-y-0.5">Become a Design Partner <ArrowRight className="h-4 w-4 transition-transform group-hover:translate-x-1" /></a></div></Reveal>
          </div>
        </section>}
